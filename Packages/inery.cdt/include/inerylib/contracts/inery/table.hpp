...
#include "../../core/inery/varint.hpp"

#include "../../core/inery/key_utils.hpp"

#include <algorithm>
#include <cctype>
#include <functional>

#warning "inery::kv::table is designated as `alpha` and should not be used in production code"
...
#define INERY_CDT_GET_RETURN_T(value_class, index_name) std::decay_t<decltype(std::invoke(&value_class::index_name, std::declval<const value_class*>()))>
...
#define KV_NAMED_INDEX(index_name, member_name)                                                                        \
   index<INERY_CDT_GET_RETURN_T(value_type, member_name)> member_name{inery::name{index_name}, &value_type::member_name};
...
inline key_type make_prefix(inery::name table_name, inery::name index_name, uint8_t status = 1) {
...
#if INERY_CDT_DOXYGEN
/**
 * @brief A function for converting types to the appropriate binary representation for the INERY Key Value database.
...
template<typename T, inery::name::raw TableName>
class table;
...
namespace internal {
   ...
   public:
      inery::name index_name;
      inery::name table_name;
      inery::name contract_name;
   ...
   protected:
      ...
      index_base(inery::name index_name, KF&& kf, T*) : index_name{index_name} {
    ...
    protected:
      ...
      void put(const void* value, void* old_value,
         ...
         for (const auto& idx : secondary_indices) {
            ...
            if (!primary_key_found) {
               inery::check(!sec_found, "Attempted to store an existing secondary index.");
               ...
               if (sec_found) {
                  ...
                  inery::check(copy_size == tbl_key.size() && res == 0, "Attempted to update an existing secondary index.");
   ...
   inline void index_base::get(const key_type& key, void* ret_val, void (*deserialize)(void*, const void*, std::size_t)) const {
      ...
      if (!is_primary) {
         ...
         inery::check(success, "failure getting primary key");
   ...
   public:
     ...
     void value(void* val, void (*deserialize)(void*, const void*, std::size_t)) const {
         ...
         inery::check(itr_stat != status::iterator_end, "Cannot read end iterator");
         ...
         inery::check(static_cast<status>(stat) == status::iterator_ok, "Error reading value");
         ...
         if (!is_primary) {
            ...
            inery::check(success, "failure getting primary key in `value()`");
      ...
      key_type key() const {
         ...
         inery::check(static_cast<status>(stat) == status::iterator_ok, "Error getting key");

         return inery::key_type{(char*)buffer, actual_value_size};
      }
 ...
 * @ingroup keyvaluetable
 * 
 * @brief Defines an INERY Key Value Table
 * @details INERY Key Value API provides a C++ interface to the INERY Key Value database.
...
private:
   ...
   public:
      ...
      iterator& operator++() {
         inery::check(itr_stat != status::iterator_end, "cannot increment end iterator");
      ...
      iterator& operator--() {
         ...
         inery::check(itr_stat != status::iterator_end, "decremented past the beginning");
         return *this;
      }
   ...
   public:
      ...
      reverse_iterator& operator++() {
         inery::check(itr_stat != status::iterator_end, "incremented past the end");
         itr_stat = static_cast<status>(detail::itr_prev(itr));
         return *this;
      }
...
public:
    ...
    * @ingroup keyvaluetable
    *
    * @brief Defines an index on an INERY Key Value Table
   ...
   public:
      ...
      index(inery::name name, KF&& kf) : index_base{name, kf, (T*)nullptr} {
      ...
      T operator[](const K& key) const {
         auto opt = get(key);
         inery::check(opt.has_value(), "Key not found in `[]`");
         return *opt;
      }
    ...
    */
   void put(const T& value, inery::name payer) {
      T old_value;
...
protected:
   ...
   void init(inery::name contract, PrimaryIndex& prim_index, SecondaryIndices&... indices) {
      validate_types(prim_index);
      (validate_types(indices), ...);

      contract_name = contract;
      table_name = inery::name{TableName};

   ...
   */
   ...
   struct kv_parameters {
      ...
      * @brief The maximum number of iterators
      ...
      INELIB_SERIALIZE( kv_parameters,
                        (max_key_size)
    ...
    */
   inline void set_kv_parameters(const inery::kv_parameters& params) {
      ...
      char buf[sizeof(uint32_t) + sizeof(inery::kv_parameters)];
      inery::datastream<char *> ds( buf, sizeof(buf) );
   ...
   /**
    *  Proposes a schedule change using the legacy master key format
    *
    *  @ingroup privileged
    ...
    *  @param masters - vector of master keys
    *
    ...
    *  @ingroup privileged
    *  @note Once the block that contains the proposal becomes irreversible, the schedule is promoted to "pending" automatically. Once the block that promotes the schedule is irreversible, the schedule will become "active"
    *  @param masters - vector of master authorities
    *
    *  @return an optional value of the version of the new proposed schedule if successful
    */

#pragma once
#include "../../core/inery/context.hpp"
...
namespace inery::kv {
   ...
   namespace detail {
       ...
       * 
       * Note that this type in `inery::kv::map` is defined as `inery::kv::map::elem_t`.
       */
       ...
       * @ingroup keyvaluemap
       * 
       * @brief This struct represents the iterator for the `inery::kv::map` data type.
       ...
       * 
       * Note that this iterator type in `inery::kv::map` is defined as `inery::kv::map::iterator_t`.
       * There is also a reverse iterator available, it is defined as `inery::kv::map::reverse_iterator_t`.
   ...
   } // namespace inery::kv::detail

   /**
    * @ingroup keyvaluemap
    *
    * @brief Defines an INERY Key Value Map
    ...
    */
   template <inery::name::raw TableName, typename K, typename V, inery::name::raw IndexName="map.index"_n>
   class [[inery::table]] map {
      public:
         ...
         static const key_type& prefix() {
            static key_type prfx = inery::detail::const_pack(magic, table_name, index_name);
            return prfx;
         }

 ...
 *
 * This is exactly equivalent to calling `set_proposed_masters_ex(0, master_data, master_data_size)`
 *
 ...
 * @param master_data - packed data of produce_keys in the appropriate master schedule order
 * @param master_data_size - size of the data buffer
 *
 * @return -1 if proposing a new master schedule was unsuccessful, otherwise returns the version of the new proposed schedule
 */
__attribute__((inery_wasm_import))
int64_t set_proposed_masters( char *master_data, uint32_t master_data_size );

/**
 * Proposes a schedule change with extended features
 *
 * Valid formats:
 * 0 : serialized array of master_key's. using this format is exactly equivalent to `set_proposed_masters(master_data, master_data_size)`
 * 1 : serialized array of master_authority's
 *
 * @note Once the block that contains the proposal becomes irreversible, the schedule is promoted to "pending" automatically. Once the block that promotes the schedule is irreversible, the schedule will become "active"
 * @param master_data_format - format of the master data blob
 * @param master_data - packed data of representing the master schedule in the format indicated.
 * @param master_data_size - size of the data buffer
 *
 * @return -1 if proposing a new master schedule was unsuccessful, otherwise returns the version of the new proposed schedule
 */
__attribute__((inery_wasm_import))
int64_t set_proposed_masters_ex( uint64_t master_data_format, char *master_data, uint32_t master_data_size );
